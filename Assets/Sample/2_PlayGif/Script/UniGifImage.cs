...
public class UniGifImage : MonoBehaviour
{
    ...
    public IEnumerator SetGifFromUrlCoroutine(string url, bool autoPlay = true)
    {
        ...
        else
        {
            ...
            //file:///表示本地文件传输协议，File协议主要用于访问本地计算机中的文件，就如同在Windows资源管理器中打开文件一样
            path = Path.Combine("file:///" + Application.streamingAssetsPath, url);
        }

        // Load file
        //var path = new System.Uri(Path.Combine(Application.streamingAssetsPath, ""));//new System.Uri可以自动拼接不同平台需要的协议，如PC file:/// ,Web http://,Android jar:file:///
        //UnityWebRequest request = UnityWebRequest.Get(path.AbsoluteUri);
        ...
        yield return StartCoroutine(UniGif.GetTextureListCoroutine(request.downloadHandler.data, (gifTexList, loopCount, width, height) =>
        {
            ...
            if (gifTexList != null)
            {
                LoadingGif(gifTexList, loopCount, width, height);
        ...
        yield return StartCoroutine(UniGif.GetTextureListCoroutine(bt, ....*/
    }

    public void LoadingGif(List<UniGif.GifTexture> gifTexList,int loopCount,int width,int height)
    {
